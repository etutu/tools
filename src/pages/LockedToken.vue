...
<script>
import LockedToken from '../web3/lockedTokenWeb3'
export default {
    name: 'LockedToken',
    data() {
        return {
    ...
    methods: {
        ...
        initWeb3() {
            this.$nextTick(() => {
                //先执行初始化web3
                LockedToken.initWeb3().then(() => {
                    // 等待web3初始化完成后执行web3方法
                    this.normalLocksForUser()
                });
            });
        },
        ...
        normalLocksForUser() {
            let that = this
            LockedToken.normalLocksForUser().then(function (myLocks) {
        ...
        editLock(id, unlockDate) {
            ...
            LockedToken.editLock(id, timestamp).then(function (receipt) {
        ...
        unlock(id) {
            LockedToken.unlock(id).then(function (receipt) {
                console.log('receipt:', receipt)
                alert('提取成功')
            }).catch((error) => {
                console.log('error:',error.error.message);  // 输出错误
